 ...
 * Williams Navbox
 * 
 * Use javascript to match a string typed into the search box against a set of keywords and page titles. 
 * Each keyword will be associated with one or more pages (ie, a URL and associated page title). 
 * Display all pages that match the string.
 * 
 * @package WMS_Navbox
 *
 * @uses    plugin    Flexiform
 * 
 * @author  Williams Web Team
 * @version 1.0.0
 */
/*
Plugin Name: Williams Navbox
Plugin URI: http://williams.edu
Description: Use javascript to match a string typed into the search box against a set of keywords and page titles. 
Version: 1.0.0
Author: Williams Web Team
Author URI: http://williams.edu
License: GPL3

Copyright Williams Web Team  (email : webteam@williams.edu)
...
class WMS_Navbox {
    var $namespace = "wms-navbox";
    var $friendly_name = "WMS Navbox";
    var $version = "1.0.0";
     ...
     * 
     * @uses add_action()
     * @uses WMS_Navbox::wp_register_scripts()
     * @uses WMS_Navbox::wp_register_styles()
     */
    function __construct() {
        ...
        $libs = glob( WMSNAVBOX_DIRNAME . '/lib/*.php' );
        foreach( $libs as $lib ) {
            include_once( $lib );
        }
         ...
         */
        load_theme_textdomain( $this->namespace, WMSNAVBOX_DIRNAME . '/languages' );
     ...
     */
    static public function _sanitize( $str ) {
    ...
    function admin_options_page() {
        ...
        include( WMSNAVBOX_DIRNAME . "/views/view-options.php" );
    }
    ...
    static function instance() {
        ...
        if( !isset( $WMS_Navbox ) ) $WMS_Navbox = new WMS_Navbox();
    }
	...
	function plugin_action_links( $links, $file ) {
		if( $file == plugin_basename( WMSNAVBOX_DIRNAME . '/' . basename( __FILE__ ) ) ) {
            $old_links = $links;
            $new_links = array(
     ...
     * form processor.
     * 
     * @uses WMS_Navbox::_admin_options_update()
     */
    function route() {
    ...
    function wp_register_scripts() {
        // Admin JavaScript
        wp_register_script( "{$this->namespace}-admin", WMSNAVBOX_URLPATH . "/js/admin.js", array( 'jquery' ), $this->version, true );
    }
    ...
    function wp_register_styles() {
        // Admin Stylesheet
        wp_register_style( "{$this->namespace}-admin", WMSNAVBOX_URLPATH . "/css/admin.css", array(), $this->version, 'screen' );
    }
}
if( !isset( $WMS_Navbox ) ) {
	WMS_Navbox::instance();
...
register_activation_hook( __FILE__, array( 'WMS_Navbox', 'activate' ) );
register_deactivation_hook( __FILE__, array( 'WMS_Navbox', 'deactivate' ) );
